...
def ban(id, user_id, user):
	...
	if int(admin_status) > 3 or int(admin_status) == 3:
		...
		if ban_status == 1:
			msg(id, '✅[id' + str(user) + '|Пользователь] уже забанен.')
		elif ban_status != 1:
			sqlite_connection = sqlite3.connect('databases\MDB.db')
			cur = sqlite_connection.cursor()

			cur.execute(f"""UPDATE users SET ban_state = ? WHERE id = ?""", (1, user))
			sqlite_connection.commit()
			cur.close()
			msg(id, '[id' + str(user) + '|Пользователь] успешно забанен.')