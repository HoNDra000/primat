...
def open_case(id, user_id):
	...
	if cas < 0 or cas == 0:
		...
		if win == 1:
			...
			res = int(old) + 10000000000 #10 миллиардов
			...
			from helpmethod.readdb import get_balance
			old = get_balance(user_id)
			res = int(old) + 5000000000 #5 миллиардов
			...
			from helpmethod.readdb import get_balance
			old = get_balance(user_id)
			res = int(old) + 25000000000 #25 миллиардов
			...
			from helpmethod.readdb import get_balance
			old = get_balance(user_id)
			res = int(old) + 1000000000 #1 миллиард
			...
			from helpmethod.readdb import get_balance
			old = get_balance(user_id)
			res = int(old) + 50000000000 #50 миллиард
			...
			from helpmethod.readdb import get_balance
			old = get_balance(user_id)
			res = int(old) + 100000000000 #100 миллиард
